...
const RouteTransition = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    // Wyzwalanie animacji tylko przy zmianie trasy
    setIsTransitioning(true);
    const timer = setTimeout(() => {
      setIsTransitioning(false);
    }, 800); // Czas animacji 0.8 sekundy
  ...
  return (
    <>
      {/* Gwarantujemy renderowanie dzieci */}
      {children}

      {/* Overlay animacji - niższy z-index, aby nie zakrywał treści */}
      <div
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div
            className={`absolute inset-y-0 w-full bg-gradient-to-r from-transparent via-green-500/20 to-transparent transform transition-transform duration-800 ease-out ${
        ...
        {/* Scan line effect */}
        <div
          className={`absolute top-0 left-0 w-full h-0.5 bg-green-400 shadow-lg shadow-green-400/50 transition-all duration-800 ease-out ${
